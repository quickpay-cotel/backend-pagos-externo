...
export class PagosService {
  ...
  async generaQr(createGeneraQrDto: CreateGeneraQrDto) {
    try {
      ...
      const resultado = await this.db.tx(async (t) => {
        ...
        // registramos deudas
        const resDeudasCliente = await this.deudasClientesRepository.create(
          {
  ...
  async confirmaPagoQr(confirmaPagoQrDto: ConfirmaPagoQrDto) {
    ...
    try {
      ...
      let qrGenerado = await this.qrGerenadoRepository.findByAlias(
        confirmaPagoQrDto.alias,
      );
      if (qrGenerado.length != 1)
        throw new Error("EL QR no se ha generado desde la Empresa");
      qrGenerado = qrGenerado[0];
      // Notificar el pago via websocket
      if (Number(qrGenerado.monto) != confirmaPagoQrDto.monto)
        throw new Error("Monto no es igual");

      // verificar que el QR tenga una sola deuda, ricardo indica q de cotel sera 1 deuda  para una factura
      let deudaCliente =
        await this.deudasClientesRepository.findByDeudaClienteId(
          qrGenerado.deuda_cliente_id,
        );
      if (deudaCliente.length != 1)
        throw new Error(
          `el alias ${confirmaPagoQrDto.alias} No cuenta con deuda`,
        );
      deudaCliente = deudaCliente[0];

      const insertDatosConfirmadoQr = {
      ...
      const datosConfirmadoQr = await this.datosConfirmadoQrRepository.create(
        insertDatosConfirmadoQr,
      );

      // notificamos pagos por websoket
      this.notificationsGateway.sendNotificationToAll(
        `${confirmaPagoQrDto.alias}|pagos realizado con exito`,
      );

      // notificar por correo
      const archivo = await this.archivosRepository.findByArchivoId(
        deudaCliente.archivo_id,
      );
      const entidad = await this.entidadesRepository.findById(
        archivo.entidad_id,
      );
      const recaudador =
        await this.segUsuariosRepository.findRecaudadorByUsuariod(
          process.env.US_QUICKPAY,
        );

      return await this.db.tx(async (t) => {
        // registramos transaccion
        const insertTransaccionCobro = {
        ...
        const transaccionCobro =
          await this.transaccionesCobrosRepository.create(
            insertTransaccionCobro,
          );
        const historicoDeudaCliente =
          await this.historicosDeudasRepository.findByDeudaClienteId(
            qrGenerado.deuda_cliente_id,
          );
        const insertCobroCliente = {
        ...
        await this.historicosDeudasRepository.update(
          { estado: "COBRADO" },
          historicoDeudaCliente.historico_deuda_id,
        );
        await this.deudasClientesRepository.delete(
          deudaCliente.deuda_cliente_id,
        );
