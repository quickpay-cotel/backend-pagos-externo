...
export class EntidadesController {
  ...
  @Get("logo/:pEntidadId")
  async getImage(
    @Param("pEntidadId", ParseIntPipe) pEntidadId: number,
    @Res() res: Response,
  ) {
    const routLogo =
      await this.entidadesService.getNameLogoByEntidadId(pEntidadId);
    if (routLogo) {
      const filePath = `${process.env.PATH_LOGO}${routLogo}`;
